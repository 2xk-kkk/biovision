...
def get_posts_with_counts(db, page=1, page_size=20, sort_by='create_at', sort_order='desc', tag=None):
    cursor = db.cursor()
    offset = (page - 1) * page_size
    
    # 验证排序字段
    valid_sort_columns = ['create_at', 'like_count', 'collect_count', 'view_count']
    ...
    # 构建排序 SQL
    sort_sql = f"ORDER BY posts.{sort_by} {sort_order.upper()}, posts.create_at DESC"
    
    # 构建 WHERE 条件
    where_clause = ""
    params = [page_size, offset]
    if tag:
        where_clause = "WHERE posts.tag = ?"
